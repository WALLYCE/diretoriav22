...
function ChartProgressaoCS(props) {
    ...
    useEffect(()=>{
        ...
        if(dados.length > 0){
            dados.forEach((item)=>{
            array.push([item['data'], parseInt(item['valor']), parseInt(item['valor'])])
            })
        }else{
 ...
 atualizaCidade();
    ...
    const options = {
        title: `Progessão de Serviços de ${props.nome}`,
        hAxis: {
          title: "Data",
        },
        vAxis: {
...
export default ChartProgressaoCS;