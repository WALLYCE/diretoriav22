...
function ChartVendasVendedores(props) {
    ...
    useEffect(()=>{
      async function jsons(){
        ...
        function agregaVendasCancelados(data){
          const counts =[]
          data.forEach((x) => {
       ...
       function agregaVendasHabilitados(data){
        const counts =[]
        data.forEach((x) => {
          if(x.status !== 'Cancelado'){
          counts[x['vendedor']] = (counts[x['vendedor']] || 0) + 1;
        ...
        return counts;
         ...
         const cancelados_suspensos = await agregaVendasCancelados(dados);
         const habilitados = await agregaVendasHabilitados(dados);
         const vendedores = await api.get(`/vendas/vendedores/ativos`);
       
         const mappedCancelados_Suspensos = Object.entries(cancelados_suspensos).map(([k,v]) => [`${k}`,parseInt(`${v}`)]);
       
         const mappedHabilitados = Object.entries(habilitados).map(([k,v]) => [`${k}`,parseInt(`${v}`)]);
     
         var vendedores_ativos = [];
  

         vendedores.data.forEach((item)=>{
          vendedores_ativos.push(removeTag(item.nome))
         })
         ...
         for(var m=0; m<vendedores_ativos.length; m++){
          var cancelamentos = 0;
          var hab = 0;
          for(var z=0; z<mappedCancelados_Suspensos.length; z++){
            if(mappedCancelados_Suspensos[z][0] == vendedores_ativos[m]){
              cancelamentos = mappedCancelados_Suspensos[z][1];
              break;
            }
          }

          for(var x=0; x<mappedHabilitados.length; x++){
            if(mappedHabilitados[x][0] == vendedores_ativos[m]){
              hab = mappedHabilitados[x][1];
              break;
            }
          }
          var stringhab ='';
          var stringcancelamentos='';
          if(hab!=0){
            stringhab = hab.toString();
          }
          if(cancelamentos!=0){
            stringcancelamentos = cancelamentos.toString();
          }
          var total = hab+cancelamentos;
          result.push([vendedores_ativos[m]+'('+total+')', hab, hab,  cancelamentos, stringcancelamentos])
         }
         
         const resultado = await result.sort()
     
          result.unshift([ 'Cidade', "Habilitado", { role: "annotation", type: "string" },"Cancelados", { role: "annotation", type: "string" },])
    ...
    useEffect(()=> {
     setBackdrop(!backdrop)
     api.post(`/vendas/vendedores`,{ data_inicial: dataInicial.split('T')[0], data_final: dataFinal.split('T')[0] })
        .then((result)=> {
      ...
      const chartEvents = [
        {
          ...
          callback({ chartWrapper }) {
            ...
            var ColSelected = grafico[0][chartWrapper.getChart().getSelection()[0].column]
           ...
           var modal = []; 
            ...
            dados.map((element)=>{
             const status = '';
             if(ColSelected == 'Cancelados'){
              if(Object.values(element)[0] === itemSelected.split('(')[0] && element.status === 'Cancelado'){
