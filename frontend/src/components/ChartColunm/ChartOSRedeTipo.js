...
function ChartOsRedeTipo(props) {
    ...
    useEffect( ()=>{
      ...
      const counts=[];
        (dados && dados.forEach((x) => {
          counts[x['descricao']] = (counts[x['descricao']] || 0) + 1;
        }))
        return counts;
       }
        ...
        jsons().then(async (counts)=> {
          ...
          novo.unshift([ 'descricao', "Quantidade", { role: "annotation", type: "string" }, {role: "style" }])
  ...
  const atualizaDados = async()=>{
    setBackdrop(true)
    const resultado = await api.post('/relatorio_tecnicos_rede',{ data_inicial: dataInicial.split('T')[0], data_final: dataFinal.split('T')[0] })
          ...
          callback({ chartWrapper }) {
           ...
           var modal = []; 
            dados.map((element)=>{
             ...
             if(Object.values(element)[2].includes(itemSelected)){
...
export default ChartOsRedeTipo;