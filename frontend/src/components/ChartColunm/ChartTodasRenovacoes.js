...
function ChartTodasRenovacoes(props) {
    ...
    useEffect(()=> {
       setBackdrop(!backdrop)
       var result =[];
      api.post(`/upgrade/cidade`,{ data_inicial: dataInicial.split('T')[0], data_final: dataFinal.split('T')[0] })
        .then(upgrade => {      
           api.post(`/renovacoes/cidade`,{ data_inicial: dataInicial.split('T')[0], data_final: dataFinal.split('T')[0] })
           .then(renovacoes=>{
            api.post(`migracoes/cidade`,{ data_inicial: dataInicial.split('T')[0], data_final: dataFinal.split('T')[0] })
            .then((migracoes)=>{
