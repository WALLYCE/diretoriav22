...
function ChartOSTipoCidade() {
    ...
    const [cidadeMenu, setCidadeMenu] = React.useState('Geral')
    ...
    useEffect(()=> {
      setBackdrop(!backdrop)
     api.post(`/os_realizada_concluida_map/cidade`, { data_inicial: dataInicial.split('T')[0], data_final: dataFinal.split('T')[0], cidade: cidadeMenu})
        .then(result => {
           setDados(result.data);
           });
        }, [menuRapido, cidadeMenu])
    ...
    const handleChangeCidade = (evento) =>{
        setCidadeMenu(evento.target.value)
    }
    ...
    const handleFiltrarClick = () =>{
      setBackdrop(!backdrop)
      api.post(`/os_realizada_concluida_map/cidade`, { data_inicial: dataInicial.split('T')[0], data_final: dataFinal.split('T')[0] })
      .then(result => {
         setDados(result.data);
         });
    }
...
export default ChartOSTipoCidade;