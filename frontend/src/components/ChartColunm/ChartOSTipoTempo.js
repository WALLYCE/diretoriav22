...
function ChartOSTipoTempo(props) {
      ...
      const foraPrazo =[];
        (dados && dados.forEach((x) => {
          if(x['situacao'] === 'Dentro do tempo estipulado'){
            dentroPrazo[x['tipo_os']] = (dentroPrazo[x['tipo_os']] || 0) + 1;
          }else{
            foraPrazo[x['tipo_os']] = (foraPrazo[x['tipo_os']] || 0) + 1;
          }
       ...
       console.log(final)
        ...
        jsons().then(async (counts)=> {
          ...
          novo.unshift([ 'Tipo', "Dentro do Prazo",{ role: "annotation", type: "string" }, "Fora do Prazo",{ role: "annotation", type: "string" }])
    ...
    useEffect(()=> {
      ...
      api.post('/relatorio_tecnicos_rede/ordem_servico/tempo_total',{ data_inicial: dataInicial.split('T')[0], data_final: dataFinal.split('T')[0] })
        .then(result => {
    ...
    const options = {
      title: "Ordem de Serviço Tempo Total - desde Atendimento",
      vAxis: { title: "Quantidade" },
      hAxis: { title: "Tipo O.S", 
      textStyle : {
          ...
          callback({ chartWrapper }) {
           ...
           var modal = []; 
            dados.map((element)=>{
             ...
             if(Object.values(element)[1] === itemSelected){
...
export default ChartOSTipoTempo;