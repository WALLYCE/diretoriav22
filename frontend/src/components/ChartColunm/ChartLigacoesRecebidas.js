...
function ChartLigacoesRecebidas(props) {
    ...
    useEffect( ()=>{
      ...
      var forameta = 0;
        ...
        (dados && dados.forEach(async (x) => {
          if(parseInt(x['espera_fila'])>temax){
            temax = parseInt(x['espera_fila']);
          }
          tt += parseInt(x['espera_fila']);
          if(parseInt(x['espera_fila'])>=20){
            forameta ++;
          }
          ...
          if(!existe){
            if(x['avaliacao'] != "sem nota" &&  x['avaliacao']!='' && x['tempo_ligacao'] != '0' ){
            ...
            meu_array.map((y)=>{
              ...
              if(y.nome == x['Ramal']){
                ...
                if(x['avaliacao'] != "sem nota" &&  x['avaliacao']!="" && x['avaliacao']!= null && x['tempo_ligacao'] != '0'){
  ...
  return (
    ...
    <Grid container mt={1} mb={1} spacing={1}>
        <Grid item xs={3} md={3}  sm={3}>
         <Cartao titulo={'Ligações Recebidas'} icone={'Ligações Recebidas'} valor={quantidade} color={'green'}/>
        </Grid>
        <Grid item xs={3} md={3}  sm={3}>
         <Cartao titulo={'Tempo de espera Médio'} icone={'Tempo de Espera'} valor={((tempoEsperaTotal/quantidade).toFixed(2))+(' (s)')} dados={[{nome: 'Tempo espera Max' , valor: tempoEsperaMax+(' (s)')}]} color={'blue'}/>
        </Grid>
        <Grid item xs={3} md={3}  sm={3}>
         <Cartao titulo={'Nível de Serviço'} icone={'Nível de Serviço'} valor={parseFloat(((quantidade-tempoForaMeta)*100)/(quantidade)).toFixed(2) + (' %')}  color={'purple'}/>
        </Grid>
        <Grid item xs={3} md={3}  sm={3}>
...
export default ChartLigacoesRecebidas;