...
function ChartProspectoVendedor(props) {
    ...
    useEffect( ()=>{
      ...
      const naorealizados=[];
        (dados && dados.forEach((x) => {
          ...
          naorealizados[x['vendedor']] = (naorealizados[x['vendedor']] || 0) + 1;
      
        }))
         const realizados =[];

        (dados && dados.forEach((x) => {
          if(x['prospecto'] ==='Prospecto foi convertido em cliente.'){
          realizados[x['vendedor']] = (realizados[x['vendedor']] || 0) + 1;
          }
        }))

        var final = [];
        for(var i in naorealizados){
          var achou = false;
          for(var j in realizados){
            if(i === j){
              achou = true;
              var valor = [naorealizados[i], realizados[j]];
              final[i] = valor
              
            }
          }
          if(achou === false){
            final[i]  = [naorealizados[i], 0]
          }
        }
       console.log(final)
        ...
        jsons().then(async (counts)=> {
          ...
          novo.unshift([ 'Vendedor', "Prospecto",{ role: "annotation", type: "string" }, "Convertidos",{ role: "annotation", type: "string" }])
    ...
    useEffect(()=> {
      ...
      api.post('/prospecto_completo',{ data_inicial: dataInicial.split('T')[0], data_final: dataFinal.split('T')[0] })
        .then(result => {
...
export default ChartProspectoVendedor;