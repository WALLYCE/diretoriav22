...
function ChartOSRemarcadas(props) {
    ...
    useEffect( ()=>{
      ...
      const counts=[];
        (dados && dados.forEach((x) => {
          counts[x['reagendamentos']] = (counts[x['reagendamentos']] || 0) + 1;
        }))
        return counts;
       }
    ...
    useEffect(()=> {
      ...
      api.post('/reagendamento_os_data',{ data_inicial: dataInicial.split('T')[0], data_final: dataFinal.split('T')[0] })
        .then(result => {
...
export default ChartOSRemarcadas;