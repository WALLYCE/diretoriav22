...
import ModalAtendimentosSac from "../ModalAtendimentosSac/ModalAtendimentosSac";

function ChartAtendimetosSacTipo(props) {
    ...
    useEffect(()=> {
      ...
      api.post('/atendimentos_sac',{ data_inicial: dataInicial.split('T')[0], data_final: dataFinal.split('T')[0] })
        .then(result => {
           result.data.map((item) =>{
              if(item['colaborador']){
              item['colaborador'] = removeTag(item['colaborador'])
              }
      ...
      const chartEvents = [
        {
          ...
          callback({ chartWrapper }) {
            ...
            var count = 0;
             ...
             while(cabecalho[id]!=props.titulo){
              id++;
            }
            console.log(cabecalho[id])
            dados.map((element)=>{
        
             if(removeTag(Object.values(element)[id]) === itemSelected || Object.values(element)[id] === itemSelected){
              const minimodal = element;
              minimodal['id'] = count;
  ...
  return (
     ...
     <ModalAtendimentosSac status={modalStats} label={labelModal} dados={dadosModal} event={()=>{setModalStats(false)}}></ModalAtendimentosSac>
...
export default ChartAtendimetosSacTipo;